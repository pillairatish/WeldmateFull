  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <Text style={{ color:'black', fontSize:20}}> My Orders</Text>
              <ScrollView style={styles.dataWrapper}>
                    ...
                    this.state.orderDetails.map((rowData, index) => (
                        <View style={{borderColor:"lightgrey" ,borderWidth:1 ,margin:10, flexDirection:'row'}}>
                          <Text style={{ color:'black'}}> Bill : {rowData.BillNo} </Text>
                          <Text style={{ color:'black'}}> dated: {moment(rowData.BillDate).format('DD-MM-YYYY')} </Text>
                        <View>
