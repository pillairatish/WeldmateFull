...
export default class SalesReport extends React.Component {
  ...
  refresh(){
    ...
    var ffulldate =  [fromdate.getFullYear(),
     ...
     this.setState({data:[]})
     fetch('http://weldmateapi.wiztechsolutions.co.in/api/Sales?fromdate='+ffulldate +'&todate='+tfulldate)
    .then((response) => response.json())
                ...
                <View style={styles.tablecontainer}> 
                  <Table>
                    ...
                    <Button title="Edit" onPress={() => {
                      this.props.navigation.navigate('Sales',{rowData: rowData1, mode:'Edit' });
                      }} />
                      :<Text/>
                    }
                    
                    {
                      rowData1.SalesInvoiceDetail.map((rowData, index) => (
